...
extern "C" {
    ...
    double evaluate(const double* vector) {
        double result = 0.0;
        for(int i=0; i<DIMENSIONS; ++i) result += vector[i]*vector[i];
        // Gallagher canonical single peak fallback without random rotation/shifts
        return 10.0 - 10.0 * exp(-0.5 * result);
    }
}
